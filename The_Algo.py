...
def find_arbitrage_opportunity(exchange_rates, currencies, buy_fees, sell_fees, start_currency):
    currencies = sorted(set(sum([(currency1, currency2) for currency1, currency_rates in exchange_rates.items() for currency2 in currency_rates], ())))
    ...
    for i in range(n_currencies):
        for j in range(i + 1, n_currencies):
            ...
            if exchange_rate is not None:
                ...
                # exchange_rates_with_fees[j][i] = -np.log(1 / ((1 - sell_fee) * exchange_rate * (1 + buy_fee)))
            else:
                reverse_rate = exchange_rates.get(currencies[j], {}).get(currencies[i], None)

                if reverse_rate is not None:
                    # exchange_rates_with_fees[i][j] = np.log(1 / ((1 - buy_fee) * reverse_rate * (1 + sell_fee)))
                    ...
                    # exchange_rates_with_fees[j][i] = -np .log((1 - sell_fee) * reverse_rate * (1 + buy_fee))
    print(exchange_rates_with_fees)
